...
const StyledLayoutItem = styled(props => {
    ...
    if (props.inset && formItemProps.validateStatus) {
        finalHelpInfo = (
            <div className="mega-layout-item-inset-help">
                {formItemProps.help[0]}
            </div>
        );
    }

    const finalFormItem = cls => (
        <Form.Item className={cls} {...formItemProps}>
            <div className="mega-layout-item-content">
                {addonBefore ? (
                    <p className="formily-mega-item-before">{addonBefore}</p>
                ) : null}
                {children}
                {addonAfter ? (
                    <p className="formily-mega-item-after">{addonAfter}</p>
                ) : null}
            </div>
        </Form.Item>
    );
        ...
        <Layout
            ...
            children={layout => {
                ...
                if (label) {
                    ...
                    const labelWidth = computeAttr(
                        layoutProps.labelWidth,
                        context.labelWidth,
                        -1,
                    );
                    const wrapperWidth = computeAttr(
                        layoutProps.wrapperWidth,
                        context.wrapperWidth,
                        -1,
                    );
                    const labelCol = computeAttr(
                        layoutProps.labelCol,
                        context.labelCol,
                        -1,
                    );
                    const wrapperCol = computeAttr(
                        layoutProps.wrapperCol,
                        context.wrapperCol,
                        -1,
                    );
                    const labelAlign = computeAttr(
                ...
                let ele = (
                    ...
                    >
                        <div className="mega-layout-container-wrapper">
                            {addonBefore ? (
                                <p className="mega-layout-container-before">
                                    {addonBefore}
                                </p>
                            ) : null}
                            <div
                            ...
                            {addonAfter ? (
                                <p className="mega-layout-container-after">
                                    {addonAfter}
                                </p>
                            ) : null}
                        </div>
...
const MegaLayoutItem = props => {
    ...
    const megaProps =
        (schemaChildren ? others['x-mega-props'] : others['mega-props']) || {};
...
const FormMegaLayout = createVirtualBox<
    Omit<ILayoutProps, 'children'> & { children: React.ReactNode }
>('mega-layout', MegaLayout);

export { MegaLayout, MegaLayoutItem, FormMegaLayout };
