...
export const computeAntdStyleBase = (props, debug?: boolean) => {
    ...
    result.labelAlignStyle = `
        & > .ant-form-item-label {
            text-align: ${
                labelAlign !== 'top' ? labelAlign || 'right' : 'left'
            };
        }
    `;
        ...
        result.widthStyle = `
            ...
            & > .ant-form-item-label{
                ${
                    ...
                    flex: ${
                        labelAlign !== 'top' ? `0 0 ${labelWidth}px` : 'initial'
                    };
                    `
                        : ''
                }
    ...
    const gridContainerStyle = (nested?: boolean) => {
        ...
        const frStyle =
            !enableSafeWidth || nested ? '1fr' : 'minmax(100px, 1fr)';
        const containerStyle =
            !disabledResponsive && responsive
                ? `
            @media (max-width: 720px) {
                grid-template-columns: repeat(${
                    !enableSafeWidth || autoRow ? s : 'auto-fit'
                }, ${frStyle});
            }
            
            @media (min-width: 720px) and (max-width: 1200px) {
                grid-template-columns: repeat(${
                    !enableSafeWidth || autoRow ? m : 'auto-fit'
                }, ${frStyle});
            }
            @media (min-width: 1200px) {
                grid-template-columns: repeat(${
                    !enableSafeWidth || autoRow ? lg : 'auto-fit'
                }, ${frStyle});
            }
        `
                : `
            grid-template-columns: repeat(${
                !enableSafeWidth || autoRow ? columns : 'auto-fit'
            }, ${frStyle});
        `;
        return `
            display: grid;
